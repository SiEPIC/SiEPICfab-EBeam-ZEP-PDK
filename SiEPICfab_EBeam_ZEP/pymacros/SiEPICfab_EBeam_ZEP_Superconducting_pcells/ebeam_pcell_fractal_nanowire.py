...
from pya import *
import numpy as np
class ebeam_pcell_fractal_nanowire(pya.PCellDeclarationHelper):
  def __init__(self):
    super(ebeam_pcell_fractal_nanowire, self).__init__()
  ...
  def display_text_impl(self):
    return "NbTiN fractal(w" + ('%.3f' % int(self.nw_width)) +"_scale"+('%.3f' % int(self.radius_scale))+ "_l" + ('%.3f' % int(self.l)) + ")"
  ...
  def produce_impl(self):
    ...
    theta_l2 = np.linspace(np.pi / 4, 4 / 4 * np.pi, n)
    # Angular spans for each arc segment
    ...
    # Midline radius (average of inner and outer arcs)
    r_mid = (r1 + r2) / 2
    ...
    L_r1   = L_l1  # mirror of left
    L_r2   = L_l2

    # Total centerline path length
