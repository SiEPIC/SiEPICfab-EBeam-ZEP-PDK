...
class ebeam_pcell_symmetric_nanobeam_side_coupled(pya.PCellDeclarationHelper):
    """
    Input: length, width
    """

    def __init__(self):
        super(ebeam_pcell_symmetric_nanobeam_side_coupled, self).__init__()
        ...
        self.param("w", self.TypeDouble, "Waveguide width (microns)", default=0.35)
        self.param("s", self.TypeDouble, "Cavity length (microns)", default=5)
        self.param("wg_l", self.TypeDouble, "Straight Waveguide length (microns)", default=5)

        self.param("n2", self.TypeInt, "Number of max radii holes", default=10)
        self.param("n1", self.TypeInt, "Number of tapered holes ", default=10)
        ...
        self.param("coupler_length", self.TypeDouble, "length of straight section of coupler (microns)", default=2.5)
        self.param("coupler_width", self.TypeDouble, "width of bus waveguide (microns)", default=0.35)
    ...
    def produce_impl(self):
        ...
        coupler_gap = self.coupler_gap / dbu
        coupler_length = self.coupler_length / dbu
        coupler_width = self.coupler_width / dbu
        ...
        # add the straight section of the bus waveguide
        clad_offset = 2 /dbu
        Si_straight = Region()
        Si_straight.insert(Box(-coupler_length/2, w/2 + coupler_gap, coupler_length/2, w/2 + coupler_gap + coupler_width))
        self.cell.shapes(LayerSiN).insert(Si_straight)
        Si_straight_clad = Region()
        Si_straight_clad.insert(Box(-coupler_length/2, w/2 + coupler_gap - clad_offset, coupler_length/2, w/2 + coupler_gap + coupler_width + clad_offset))
        self.cell.shapes(LayerCladN).insert(Si_straight_clad)      


        # Add the sbends of the bus waveguide
        from SiEPIC.utils.layout import layout_waveguide_sbend
        from math import pi, acos, sin
        clad_width = 2 / dbu
        h = 5 / dbu
        rad = 10 / dbu
        theta = acos(float(rad-abs(h/2))/rad)*180/pi
        bend_length = int(2*rad*sin(theta/180.0*pi))
        layout_waveguide_sbend(self.cell, LayerSiN, pya.Trans(Trans.R0, coupler_length/2,w/2 + coupler_gap+coupler_width/2), coupler_width, rad, h, bend_length)
        layout_waveguide_sbend(self.cell, LayerCladN, pya.Trans(Trans.R0, coupler_length/2,w/2 + coupler_gap+coupler_width/2), coupler_width+clad_width*2, rad, h, bend_length)

        layout_waveguide_sbend(self.cell, LayerSiN, pya.Trans(Trans.M90, -coupler_length/2,w/2 + coupler_gap+coupler_width/2), coupler_width, rad, h, bend_length)
        layout_waveguide_sbend(self.cell, LayerCladN, pya.Trans(Trans.M90, -coupler_length/2,w/2 + coupler_gap+coupler_width/2), coupler_width+clad_width*2, rad, h, bend_length)

        # add straights to bus (if needed)
        if 2*beam_l > (coupler_length + 2*bend_length):
            # left side straight
            clad_offset = 2 /dbu
            Si_straight = Region()
            Si_straight.insert(Box(-beam_l, w/2 + coupler_gap + h, -bend_length - coupler_length/2, w/2 + coupler_gap + h + coupler_width))
            self.cell.shapes(LayerSiN).insert(Si_straight)
            Si_straight_clad = Region()
            Si_straight_clad.insert(Box(-beam_l, w/2 + coupler_gap + h - clad_offset, -bend_length - coupler_length/2, w/2 + coupler_gap + h + coupler_width + clad_offset))
            self.cell.shapes(LayerCladN).insert(Si_straight_clad)
            # right side straight
            clad_offset = 2 /dbu
            Si_straight = Region()
        ...
        yp1 = w/2 + coupler_gap + h + coupler_width /2
        xa = min(-beam_l, -coupler_length/2 - bend_length)
        xb = max(beam_l, coupler_length/2 + bend_length)
        ...
        dev = Box(xa, -w/2 - clad_offset, xb, w/2+coupler_gap+h+coupler_width + clad_offset)
