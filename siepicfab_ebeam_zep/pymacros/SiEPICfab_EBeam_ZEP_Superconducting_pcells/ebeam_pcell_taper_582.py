...
class ebeam_pcell_taper_582(pya.PCellDeclarationHelper):
    """
    Input: length, width
    """

    def __init__(self):
        super(ebeam_pcell_taper_582, self).__init__()
    ...
    def produce_impl(self):
       ...
       # params = params * 1e6 / dbu
        ...
        if x_all:
            mid_index = (len(x_all) // 4) + (1 if len(x_all) % 2 != 0 else 0)
            pin_x = x_all[mid_index]

            # Add left bus pin
            xp1 = 0.05 / dbu
            xp2 = -0.05 / dbu
            yp1 = 0
            pin1 = [Point(pin_x + xp1, yp1), Point(pin_x + xp2, yp1)]
            shapes(LayerPinRecN).insert(Path(pin1, w))
            shapes(LayerPinRecN).insert(Text("opt1", Trans(Trans.R0, pin_x, yp1)))

            # Add right bus pin
            pin2 = [Point(-pin_x - xp1, yp1), Point(-pin_x - xp2, yp1)]
            shapes(LayerPinRecN).insert(Path(pin2, w))
            shapes(LayerPinRecN).insert(Text("opt2", Trans(Trans.R0, -pin_x, yp1)))
